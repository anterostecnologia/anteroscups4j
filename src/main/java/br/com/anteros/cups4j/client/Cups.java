...
public class Cups {
  ...
  private static void getPrinterAttributes(String host, String userName, String printerName) throws Exception {
    if (host == null) {
      host = AnterosCupsClient.DEFAULT_HOST;
    }

    if (userName == null) {
      userName = AnterosCupsClient.DEFAULT_USER;
    }
    if (printerName == null) {
      AnterosCupsClient cupsClient = new AnterosCupsClient(host, AnterosCupsClient.DEFAULT_PORT, userName);
      printerName = cupsClient.getDefaultPrinter().getName();
    }

    AnterosCupsPrinter printer = new AnterosCupsPrinter(new URL("http://" + host + "/printers/" + printerName), printerName, false);
  ...
  private static void getJobs(String host, String userName, String printerName) throws Exception {
    ...
    AnterosCupsClient cupsClient = new AnterosCupsClient();

    // if user provided - get only jobs from this user.
    boolean myJobs = true;
    if (userName.equals(AnterosCupsClient.DEFAULT_USER)) {
      myJobs = false;
    }
    List<PrintJobAttributes> jobs = cupsClient.getJobs(
        new AnterosCupsPrinter(new URL("http://" + host + "/printers/" + printerName), printerName, false), WhichJobsEnum.ALL,
        userName, myJobs);
  ...
  private static void print(String host, String printerName, String fileName, int copies, String pages, boolean duplex,
    ...
    AnterosCupsClient cupsClient = new AnterosCupsClient(host, AnterosCupsClient.DEFAULT_PORT);
    if (printerName == null) {

      printer = cupsClient.getDefaultPrinter();
    } else {
      printer = new AnterosCupsPrinter(new URL("http://" + host + ":" + AnterosCupsClient.DEFAULT_PORT + "/printers/" + printerName),
          printerName, false);
    }
  ...
  private static void listPrintersOnHost(String hostname) throws Exception {
    ...
    List<AnterosCupsPrinter> printers = null;
    long timeoutTime = System.currentTimeMillis() + 10000;
    while (System.currentTimeMillis() < timeoutTime && printers == null) {
      try {
        AnterosCupsClient cupsClient = new AnterosCupsClient(hostname, AnterosCupsClient.DEFAULT_PORT);
